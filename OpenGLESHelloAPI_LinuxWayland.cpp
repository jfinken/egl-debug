...
bool createEGLSurface(EGLDisplay eglDisplay, EGLConfig eglConfig, EGLSurface& eglSurface)
{
	...
	eglSurface = eglCreateWindowSurface(eglDisplay, eglConfig, (EGLNativeWindowType)wlEglWindow, NULL);
	if (!testEGLError("eglCreateWindowSurface")) { return false; }
		
    printf("Created eglCreateWindowSurface\n");
	return true;
}
...
bool setupEGLContext(EGLDisplay eglDisplay, EGLConfig eglConfig, EGLSurface eglSurface, EGLContext& context)
{
    ...
    printf("Created egl context and eglMakeCurrent success\n");
	return true;
}
...
bool initializeBuffer(GLuint& vertexBuffer)
{
    ...
    printf("InitializeBuffer success\n");
	return true;
}
...
bool initializeShaders(GLuint& shaderProgram)
{
    ...
    printf("InitializeShaders success\n");
	return true;
}
...
bool render(GLuint shaderProgram, GLuint vertexBuffer, EGLDisplay& eglDisplay, EGLSurface& eglSurface)
{
	...
	for (int i = 0; i < 800; ++i)
	{
        ...
        printf("wl_display_dispatch_pending, i: %d\n", i);

		if (!renderScene(shaderProgram, vertexBuffer, eglDisplay, eglSurface)) { 
            return false; 
        }
        printf("renderScene, i: %d\n", i);
	}
	return true;
}
